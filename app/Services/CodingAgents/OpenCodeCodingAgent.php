...
class OpenCodeCodingAgent implements CodingAgent
{
    private const PERMISSIVE_PERMISSION = '"allow"';

    private const READ_ONLY_PERMISSION = '{"*":"deny","read":"allow","glob":"allow","grep":"allow","list":"allow","lsp":"allow","skill":"allow","webfetch":"allow","websearch":"allow","codesearch":"allow","edit":"deny","bash":"deny","task":"deny","question":"deny","todowrite":"deny"}';
    ...
    public function __construct(
        ...
        private ?OpenCodeJsonEventParser $jsonEventParser = null,
    ) {}

    public function run(Task $task, TaskRun $run): CodingAgentResult
    {
        return $this->executeTaskPhaseCommand(
            command: $this->buildJsonCommand(
                $this->resolveWorkspacePath($run),
            ...
            run: $run,
            successMessage: 'OpenCode implementation command completed.',
            failureMessage: 'OpenCode implementation command failed.',
            permission: self::PERMISSIVE_PERMISSION,
    ...
    {
        $result = $this->executeTaskPhaseCommand(
            command: $this->buildJsonCommand(
                $this->resolveWorkspacePath($run),
            ...
            repositoryPath: $this->resolveWorkspacePath($run),
            run: $run,
            successMessage: 'OpenCode planning command completed.',
            failureMessage: 'OpenCode planning command failed.',
    ...
    public function resumeReview(Task $task, TaskRun $run, string $feedback, int $attempt): CodingAgentResult
    {
        ...
        return $this->executeReviewCommand(
            ...
            'OpenCode review resume completed.',
            'OpenCode review resume failed.',
        );
    }
    ...
    public function resumeImplementation(Task $task, TaskRun $run, string $feedback, int $attempt): CodingAgentResult
    {
        ...
        return $this->executeTaskPhaseCommand(
            ...
            repositoryPath: $this->resolveWorkspacePath($run),
            run: $run,
            successMessage: 'OpenCode implementation resume completed.',
            failureMessage: 'OpenCode implementation resume failed.',
            permission: self::PERMISSIVE_PERMISSION,
            model: $this->resolveRunSetting($run, 'implement_model'),
            reasoningEffort: $this->resolveRunSetting($run, 'implement_reasoning_effort'),
        );
    ...
    public function captureScreenshot(Task $task, TaskRun $run): CodingAgentResult
    {
        return $this->executeTaskPhaseCommand(
            ...
            repositoryPath: $this->resolveWorkspacePath($run),
            run: $run,
            successMessage: 'OpenCode screenshot command completed.',
            failureMessage: 'OpenCode screenshot command failed.',
            permission: self::PERMISSIVE_PERMISSION,
            model: $this->resolveRunSetting($run, 'implement_model'),
            reasoningEffort: $this->resolveRunSetting($run, 'implement_reasoning_effort'),
        );
    ...
    public function smokeTestUrl(Task $task, TaskRun $run): CodingAgentResult
    {
        return $this->executeTaskPhaseCommand(
            ...
            repositoryPath: $this->resolveWorkspacePath($run),
            run: $run,
            successMessage: 'OpenCode smoke test command completed.',
            failureMessage: 'OpenCode smoke test command failed.',
            permission: self::PERMISSIVE_PERMISSION,
            model: $this->resolveRunSetting($run, 'implement_model'),
            reasoningEffort: $this->resolveRunSetting($run, 'implement_reasoning_effort'),
        );
    ...
    public function generateCommitMessage(Task $task, TaskRun $run): CodingAgentResult
    {
        $result = $this->executeTaskPhaseCommand(
            ...
            repositoryPath: $this->resolveWorkspacePath($run),
            run: $run,
            successMessage: 'OpenCode commit message command completed.',
            failureMessage: 'OpenCode commit message command failed.',
            permission: self::PERMISSIVE_PERMISSION,
    ...
    {
        $prompt = $this->buildAnalyzePrompt($inputSource, $projectSummaries);
        $repositoryPath = (string) base_path();
        $command = $this->buildPlainCommand(
            $repositoryPath,
            $prompt,
        ...
        $process->setEnv($this->environmentForStandaloneCommand($repositoryPath, self::PERMISSIVE_PERMISSION));
        $process->run();

        $stdout = trim((string) $process->getOutput());
        $stderr = trim((string) $process->getErrorOutput());
        $summary = $this->jsonEventParser()->parse($stdout);
        $invocation = new CodingAgentInvocation(
            command: $command,
            sessionId: $summary->sessionId,
            model: $summary->model ?? $this->resolveAnalyzeSourceModel(),
            reasoningEffort: $this->resolveAnalyzeSourceReasoningEffort(),
            usage: $summary->usage(),
        );

        if ($summary->hasError() || ! $process->isSuccessful()) {
            return new CodingAgentResult(
                successful: false,
                messages: $this->commandMessages('OpenCode input analysis failed.', $this->assistantOutput($summary), $stderr, $stdout),
                error: $this->failureMessage($summary, $stderr, $stdout, 'OpenCode analysis command failed.'),
                invocation: $invocation,
            );
        }

        try {
            $payload = $this->decodeStructuredJsonOutput($stdout, $summary);
        } catch (JsonException $exception) {
            return new CodingAgentResult(
                successful: false,
                messages: $this->commandMessages('OpenCode input analysis completed.', $this->assistantOutput($summary), $stderr, $stdout),
                error: 'OpenCode returned invalid task JSON: '.$exception->getMessage(),
                invocation: $invocation,
            );
        }
    ...
    private function reviewTextHasFindings(TaskRun $run, string $output): bool
    {
        ...
        $process->setTimeout(null);
        $process->setEnv($this->environmentForTaskRun($run, $repositoryPath, self::READ_ONLY_PERMISSION));
     ...
     */
    private function executeTaskPhaseCommand(
        array $command,
        ...
        $process = new Process($command, $repositoryPath);
        $process->setTimeout(null);
        $process->setEnv($this->environmentForTaskRun($run, $repositoryPath, $permission));
        $this->runInterruptibleProcess($process, $run);
        ...
        $summary = $this->jsonEventParser()->parse($stdout);
        $assistantOutput = $this->assistantOutput($summary);
        $invocation = new CodingAgentInvocation(
            command: $command,
            sessionId: $summary->sessionId,
    ...
    private function executeReviewCommand(
        TaskRun $run,
        array $command,
        string $successMessage = 'OpenCode review command completed.',
        string $failureMessage = 'OpenCode review command failed.',
    ): CodingAgentResult {
        $result = $this->executeTaskPhaseCommand(
            command: $command,
            repositoryPath: $this->resolveWorkspacePath($run),
        ...
        $reviewText = $this->primaryAgentOutput($result);
     ...
     * @return list<string>
     */
    private function buildJsonCommand(
        string $repositoryPath,
        string $prompt,
        ?string $model = null,
        ?string $reasoningEffort = null,
     ...
     * @return list<string>
     */
    private function buildPlainCommand(
        string $repositoryPath,
        string $prompt,
        ?string $model = null,
        ?string $reasoningEffort = null,
     ...
     */
    private function environmentForTaskRun(TaskRun $run, string $repositoryPath, string $permission): array
    {
        return array_merge(
            $this->context,
            [
     ...
     * @return array<string, mixed>
     */
    private function environmentForStandaloneCommand(string $repositoryPath, string $permission): array
    {
    ...
    private function failureMessage(
        ...
        if ($summary->errorEventJson !== null && $summary->errorEventJson !== '') {
            return $summary->errorEventJson;
        }
     ...
     */
    private function decodeStructuredJsonOutput(
        string $stdout,
        OpenCodeJsonEventSummary $summary,
    ): array {
        try {
            return $this->decodeJsonPayload($stdout);
        } catch (JsonException) {
            $assistantOutput = $this->assistantOutput($summary);

            if ($assistantOutput === '') {
                throw new JsonException('No JSON payload was returned.');
            }

            return $this->decodeJsonPayload($assistantOutput);
        }
    }
    ...
    private function openCodeExecutable(): string
    {
        $path = (string) Arr::get($this->context, 'PATH', '');

        foreach (array_filter(explode(PATH_SEPARATOR, $path)) as $directory) {
            $executable = rtrim($directory, DIRECTORY_SEPARATOR).DIRECTORY_SEPARATOR.'opencode';
    ...
    private function jsonEventParser(): OpenCodeJsonEventParser
    {
        return $this->jsonEventParser ??= app(OpenCodeJsonEventParser::class);
    }

    /**
     * @return list<string>
     */
    private function commandMessages(string $successMessage, string $assistantOutput, string $errorOutput, string $stdout): array
    {
        $output = $assistantOutput !== '' ? $assistantOutput : $stdout;
    ...
    private function buildAnalyzePrompt(InputSource $inputSource, array $projectSummaries): string
    {
...
- The plan must be decision complete for another implementation pass, concise by default, and include Summary, Key Changes, Test Plan, and Assumptions sections.
