...
```text
Pasted text or stored uploaded input file
...
- The app uses one task workflow status, not separate approval and AI status
  fields.
- Input sources are generic inputs, not meeting transcripts only. Users can
  paste text or upload `.txt`, `.md`, or `.pdf` files. Uploaded files are stored
  privately, and PDFs are analyzed by passing the stored file to the coding
  agent instead of requiring extracted text.
...
| id | title | original_filename | file_disk | file_path | mime_type | file_size | analysis_result | analysis_status | last_analysis_error |
|---:|---|---|---|---|---|---:|---|---|---|
| 1 | Auth Planning - 09:00 02/05/2026 | auth-planning-notes.txt | local | input-sources/auth-planning-notes.txt | text/plain | 1420 | {"task_count":3,"tasks":[{"title":"Create sign in page"}]} | completed | null |
| 2 | Launch PDF Brief | launch-brief.pdf | local | input-sources/launch-brief.pdf | application/pdf | 184320 | null | pending | null |
...
Stores private uploaded input files before task analysis. Pasted text is stored
as a private generated `.txt` file. The
...
`file_*`, `mime_type`, and `file_size` columns identify the private stored file.
The `analysis_result` column stores the AI analysis result, including extracted
tasks. PDF imports do not depend on OCR or extracted text.
...
- Users can paste text or upload `.txt`, `.md`, or `.pdf` files.
- The HTTP request stores source metadata and any uploaded file on the private
  `local` disk with `analysis_status` set to `pending`, dispatches a Laravel
  queued job, and returns immediately.
- Pasted text is stored as a generated private `.txt` file. Uploaded files are
  stored as provided.
- Stored input source files have a private preview route that streams the file
  inline for browser-native preview.
- Input analysis runs in the queued job, not in the HTTP request.
- The queued job sets `analysis_status` to `processing`, calls the
  agent-backed `TaskExtractor` PHP contract, creates extracted tasks and
  acceptance criteria, then sets `analysis_status` to `completed`.
- The coding agent receives the stored absolute file path, original filename,
  MIME type, and file size.
- Successful analysis stores the extracted task payload in `analysis_result`.
...
`CodingAgentResult` may include a JSON payload containing extracted tasks. The
`TaskExtractor` output shape remains unchanged.
...
2. **Input to Tasks**
   - Add pasted text and `.txt`, `.md`, `.pdf` upload.
   - Store input sources and uploaded files privately before analysis.
   - Add a private inline preview route for stored input source files.
   - Implement the agent-backed `TaskExtractor` contract.
   - Pass file-backed source path metadata to the coding agent for analysis.
...
- Pasted text import creates an input source and queues analysis.
- Uploaded `.txt`, `.md`, and `.pdf` files are stored on the private `local`
  disk.
- Successful analysis stores extracted task data in `input_sources.analysis_result`.
- PDF upload queues analysis without requiring extractable text.
- `.markdown` uploads remain rejected.
- The preview route streams stored files inline and returns 404 for missing
  stored files.
- The agent prompt includes stored absolute file path metadata for file-backed
  input sources.
...
- Supported uploads are `.txt`, `.md`, and `.pdf`.
- Uploaded input source files remain private on the `local` disk.
- OCR is out of scope for v1; PDF analysis uses the stored file directly.
- Browser-native inline preview is sufficient for v1.
